...
static wchar_t connection_string[] = L"Driver=SQL Server Native Client 11.0;Server=tcp:x4aoy5jfej.database.windows.net,1433;Database=Chickens;Encrypt=yes;Uid=kenny@x4aoy5jfej;Pwd=OVOdBr53#49Fbkife8vE";
...
auto main() -> int
{
    ...
    execute(s, L"drop table Hens");

    execute(s, L"create table Hens ( Id int primary key, Name nvarchar(100) not null )");

    execute(s, L"insert into Hens (Id, Name) values (1, 'Rowena'), (2, 'Henrietta'), (3, 'Constance')");

    execute(s, L"select Id from Hens where Name = 'Henrietta'");
    ...
    execute(s, L"select Name from Hens where ID = 3");
    ...
    VERIFY_(SQL_SUCCESS, SQLBindCol(s.get(),
                                    1, 
                                    SQL_C_SLONG,
                                    &id,
                                    0,
